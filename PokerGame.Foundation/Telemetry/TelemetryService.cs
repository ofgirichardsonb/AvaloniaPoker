...
{
    /// <summary>
    /// Telemetry service using Application Insights
    /// </summary>
    public class TelemetryService : ITelemetryService
    {
        ...
        public bool Initialize(string instrumentationKey)
        {
            ...
            try
            {
                ...
                string maskedKey = trimmedKey.Length > 4 
                    ? $"'{trimmedKey.Substring(0, 4)}...' (Length: {trimmedKey.Length})"
                    : $"(Length: {trimmedKey.Length})";
                    
                Console.WriteLine($"Initializing Application Insights telemetry with key: {maskedKey}");
                
                _instrumentationKey = trimmedKey;
                
                // Set connection string (preferred approach)
